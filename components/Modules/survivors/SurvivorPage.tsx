...
import { useSurvivorsData } from '@/data/survivorsData';
...
const QuoteIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className="w-10 h-10 text-blue-200">
...
export default function SurvivorPage() {
  ...
  const survivorsData = useSurvivorsData();
  const pageData = survivorsData.find(p => p.slug === slug);

  if (!pageData) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center">
        <h1 className="text-4xl font-bold text-gray-900 mb-6">Page Not Found</h1>
        <p className="text-lg text-gray-600">The requested survivor page does not exist.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="max-w-4xl mx-auto space-y-16">
          {pageData.sections.map((section, index) => (
            <motion.div key={index} variants={itemVariants} className="space-y-6">
              ...
              {section.items && !section.isQuotes && (
              ...
              {section.items && section.isQuotes && (
                <div className="grid md:grid-cols-2 gap-6 mt-8">
                   {section.items.map((quote, qIndex) => (
                      <div key={qIndex} className="bg-white p-8 rounded-2xl shadow-md border border-blue-50 relative overflow-hidden">
                        <div className="absolute top-4 right-6 opacity-20"><QuoteIcon /></div>
                        <p className="font-serif text-lg text-gray-700 italic relative z-10 leading-relaxed">{quote}</p>
                        <p className="mt-4 text-sm font-bold text-synergy-blue uppercase tracking-wider">— Patient</p>
                      </div>
                   ))}
                </div>
              )}
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="mb-20">
        <div className="bg-gradient-to-r from-orange-50 to-rose-50 p-8 md:p-16 rounded-3xl border border-rose-100 text-center shadow-sm">
           <motion.h2 variants={itemVariants} className={`${newYorkTypography.h2} border-none mb-4 text-gray-900`}>
              SHARE YOUR STORY
           </motion.h2>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="bg-blue-900 text-white py-16 md:py-20 rounded-3xl shadow-xl">
        <div className="max-w-4xl mx-auto text-center px-4">
          <motion.h2 variants={itemVariants} className={`${newYorkTypography.h1} mb-6 border-none text-white`}>
            Need guidance or a consultation?
          </motion.h2>
          <motion.p variants={itemVariants} className="text-xl text-blue-100 mb-10 max-w-2xl mx-auto leading-relaxed">
            If you or someone you care about is facing cancer, help is available. Our team is here to listen, guide, and support you at every step.
          </motion.p>
